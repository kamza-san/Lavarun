...
def setting(clock,screen,FPS,MYFONT):
    with open("./file/set.json","r") as f:
        ...
        fall_num = int(data[3].split(":")[1])
    right = Button(200,300,200,80,button,"우측이동 : "+pygame.key.name(right_num),MYFONT,0,0,0,screen)
    left = Button(200,420,200,80,button,"좌측이동 : "+pygame.key.name(left_num),MYFONT,0,0,0,screen)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if right.click(pygame.mouse.get_pos()):
                    ...
                    while out:
                        ...
                        for event in pygame.event.get():
                            if event.type == pygame.KEYDOWN:
                                jump.text = "점프 : "+event.unicode
                                jump_num = event.key
                                out = False
                                break 
                elif fall.click(pygame.mouse.get_pos()):
                    out = True
                    while out:
                        click.draw()
                        right.draw()
